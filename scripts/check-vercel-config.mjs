...
assert(
  ...
  "Vercel routes must check generated files before applying the 404 fallback",
);
assert(
  rootConfig.routes?.some(
    (route) =>
      route.src === "/(.*)" &&
      route.status === 404 &&
      route.dest === "/404.html",
  ),
  "unknown frontend routes must serve 404.html with an HTTP 404 status",
);
assert(
