...
export default function AdminInitiativesPage() {
  ...
  const handleSave = async () => {
    if (!form.title.trim() || !form.rawUrl.trim()) {
      setError(t("العنوان والرابط مطلوبان", "Title and URL are required"));
      return;
    }
    setSaving(true);
    setError("");
  ...
  const handleDelete = async (init: Initiative) => {
    if (!confirm(`${t('حذف', 'Delete')} "${init.title}"?`)) return;
    await fetch(`${API_BASE}/api/admin/initiatives/${init.id}`, { method: "DELETE", credentials: "include" });
    fetchList();
  };

  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">{t('المبادرات المجتمعية', 'Community initiatives')}</h1>
          <p className="text-sm text-muted-foreground">{t('إضافة وتعديل بطاقات المبادرات الظاهرة للمستخدمين', 'Add and edit initiative cards visible to users')}</p>
        </div>
        <button
          onClick={openNew}
      ...
      {showForm && (
        <div className="bg-card border border-border/60 rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-foreground">{editingId ? t("تعديل المبادرة", "Edit initiative") : t("مبادرة جديدة", "New initiative")}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-foreground mb-1">{t('الأيقونة (emoji)', 'Icon (emoji)')}</label>
              <input
                value={form.icon}
            ...
            <div>
              <label className="block text-xs font-medium text-foreground mb-1">{t('العنوان *', 'Title *')}</label>
              <input
                value={form.title}
                onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
                className="w-full h-10 rounded-xl border border-border bg-background px-3 text-sm focus:outline-none focus:border-primary"
                 placeholder={t('اسم المبادرة', 'Initiative name')}
              />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-foreground mb-1">{t('الوصف', 'Description')}</label>
              <textarea
                ...
                className="w-full h-20 rounded-xl border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:border-primary resize-none"
                 placeholder={t('وصف مختصر للمبادرة...', 'Brief initiative description...')}
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-foreground mb-1">{t('الرابط * (# إذا قريباً)', 'URL * (# if coming soon)')}</label>
              <input
                value={form.rawUrl}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-10 gap-3 text-muted-foreground">
         ...
         <div className="text-center py-10 text-muted-foreground text-sm">{t('لا توجد مبادرات بعد', 'No initiatives yet')}</div>
      ) : (
            ...
            <div key={init.id} className="flex items-center gap-3 bg-card border border-border/60 rounded-2xl px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className={`text-sm font-semibold ${!init.isActive ? 'text-muted-foreground line-through' : 'text-foreground'}`}>
                   ...
                   {!init.isActive && <span className="text-xs text-muted-foreground">{t('(مخفية)', '(Hidden)')}</span>}
                </div>
                 <p className="text-xs text-muted-foreground truncate" dir="ltr">{init.rawUrl}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                {init.rawUrl !== "#" && (
                 ...
                 <button onClick={() => toggleActive(init)} className="p-1.5 rounded-lg hover:bg-muted/50 text-muted-foreground hover:text-foreground" title={init.isActive ? t("إخفاء", "Hide") : t("إظهار", "Show")}>
