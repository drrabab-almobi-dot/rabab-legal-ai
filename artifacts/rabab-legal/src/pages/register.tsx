...
export default function Register() {
  ...
  const registerSchema = z
    .object({
      ...
      phone: z
        .string()
        .min(9, t("رقم الجوال قصير جداً", "Phone number is too short"))
        .regex(/^[0-9+]+$/, t("أرقام فقط", "Numbers only")),
      password: z
        .string()
        .min(
          8,
  ...
  setPageSEO({
    ...
    description: t(
      "أنشئ حسابك في RABAB LEGAL AI وابدأ في الحصول على استشارات قانونية أونلاين في الأنظمة السعودية.",
      "Create an account with RABAB LEGAL AI and begin receiving online legal consultations for Saudi laws.",
    ),
    canonical: "https://rabablegal.com/register",
  });
  ...
  const [accountType, setAccountType] = useState<"individual" | "entity">(
    "individual",
  );
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    const fullData = {
      ...
      ...(accountType === "entity"
        ? {
            entityName,
            entityCrNumber: entityCr,
            entityTaxNumber: entityTax || undefined,
          }
        : {}),
    };
    try {
      ...
      if (res.pendingVerification && res.verifyToken) {
        ...
        toast({
          ...
          description: t(
            `أُرسل رمز SMS إلى ${res.maskedPhone ?? ""}`,
            `An SMS code was sent to ${res.maskedPhone ?? ""}`,
          ),
        });
        ...
        toast({
          title: t("تم إرسال رمز التحقق", "Verification code sent"),
          description: t(
            "تحققي من بريدك الإلكتروني لإكمال إنشاء الحساب.",
      ...
      toast({
        variant: "destructive",
        title: t("فشل إنشاء الحساب", "Account creation failed"),
        description:
          err?.error ||
          err?.message ||
  ...
  const onConfirmOtp = async () => {
    if (otpCode.length !== 6) {
      toast({
        variant: "destructive",
        title: t("الرمز غير صحيح", "Incorrect code"),
        description: t(
    ...
    try {
      const res = await customFetch<{ token: string; user: any }>(
        "/api/auth/phone-verify/confirm",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ verifyToken, code: otpCode }),
        },
      );
      ...
      toast({
        ...
        description: t(
          "مرحباً بك في رباب محاميتك الرقمية",
          "Welcome to Rabab, your digital lawyer.",
        ),
      });
            ...
            <>
              <CardHeader className="text-center pb-8 pt-10">
                ...
                <p className="text-muted-foreground text-base leading-relaxed">
                  {t(
                    "انضم إلى رباب محاميتك الرقمية في الأنظمة السعودية والخليجية",
                    "Join Rabab, your digital lawyer for Saudi and GCC laws",
                  )}
                  <br />
                  RABAB LEGAL AI{" "}
                  {t("وابدأ استشاراتك", "and start your consultations")}
                </p>
              </CardHeader>
                  ...
                  {accountType === "entity" && (
                    <div className="space-y-3 p-4 bg-muted/40 rounded-xl border-2 border-blue-400/45 shadow-sm shadow-blue-400/5">
                      <p className="text-xs font-bold text-muted-foreground">
                        {t("بيانات المنشأة", "Organization details")}
                      </p>
                      <div className="space-y-2">
                        <Label htmlFor="entityName">
                          {t("اسم المنشأة", "Organization name")}{" "}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            id="entityTax"
                            value={entityTax}
                            onChange={(e) => setEntityTax(e.target.value)}
                            placeholder="300..."
                            dir="ltr"
                          />
                        </div>
