...
function getGoogleOAuthConfig(): GoogleOAuthConfig | null {
  ...
  return {
    ...
    redirectUri:
      ...
      `${DEFAULT_FRONTEND_ORIGIN}/api/auth/google/callback`,
  };
}
...
function oauthFrontendUrl(path: string, error?: string): string {
  const target = new URL(
    safeReturnTo(path),
    process.env.FRONTEND_URL?.trim() || DEFAULT_FRONTEND_ORIGIN,
  );
...
function issueToken(
  ...
  return jwt.sign(
    { userId, userRole, jti: uuidv4(), tokenVersion },
    JWT_SECRET,
 ...
 */
async function revokeVerifiedToken(
  rawToken: string,
  exp?: number,
): Promise<void> {
...
async function rollbackUndeliverableRegistration(
  userId: number,
): Promise<void> {
  ...
  await db
    .delete(phoneOtpTokensTable)
    .where(eq(phoneOtpTokensTable.userId, userId));
  await db
...
router.get("/auth/google", (req, res): void => {
  ...
  if (!config) {
    res.redirect(
      302,
      oauthFrontendUrl(safeReturnTo(req.query.returnTo), "google_unavailable"),
    );
    return;
  }
  ...
  req.session!.save((sessionError) => {
    if (sessionError) {
      logger.error(
        { err: sessionError },
        "Unable to persist Google OAuth state",
      );
      res.redirect(
        302,
        oauthFrontendUrl(
...
router.get("/auth/google/callback", async (req, res): Promise<void> => {
  ...
  if (
    !expectedState ||
    !suppliedState ||
    !sameValue(expectedState, suppliedState)
  ) {
  ...
  try {
    ...
    if (!profile)
      throw new Error("Google OAuth response did not include a profile");
    ...
    if (!user) {
      ...
      const [freePackage] = await db
        .select()
        .from(packagesTable)
        .where(eq(packagesTable.type, "free"));
      if (freePackage) {
    ...
    req.session!.save((sessionError) => {
      if (sessionError) {
        logger.error(
          ...
          "Unable to save Google OAuth session",
        );
...
router.post("/auth/register", async (req, res): Promise<void> => {
  ...
  const canVerifyBySms =
    Boolean(phone) &&
    (isSmsConfigured() || process.env.NODE_ENV !== "production");
  const canVerifyByEmail = await hasEmailVerificationChannel();

  if (!canVerifyBySms && !canVerifyByEmail) {
    res.status(503).json({
      error:
        "التسجيل غير متاح مؤقتاً لأن خدمة التحقق غير مهيأة. يرجى المحاولة لاحقاً أو التواصل مع إدارة المنصة.",
  ...
  const accountType =
    req.body.accountType === "entity" ? "entity" : "individual";
  const entityName =
    accountType === "entity"
      ? ((req.body.entityName as string | undefined) ?? null)
      : null;
  const entityCrNumber =
    accountType === "entity"
      ? ((req.body.entityCrNumber as string | undefined) ?? null)
      : null;
  const entityTaxNumber =
    accountType === "entity"
      ? ((req.body.entityTaxNumber as string | undefined) ?? null)
      : null;
  ...
  if (phone) {
    ...
    await db
      .insert(phoneOtpTokensTable)
      .values({ userId: user.id, verifyToken, code, expiresAt });

    try {
      const normalizedPhone = normalizePhoneE164(phone);
      await sendSms({
        to: normalizedPhone,
        body: `رمز التحقق في رباب: ${code} — صالح 10 دقائق`,
      });
      ...
      logger.warn(
        ...
        "[SMS] Failed to send OTP — falling back to email verification",
      );
      // Clean up phone OTP tokens since SMS delivery failed
      await db
        .delete(phoneOtpTokensTable)
        .where(eq(phoneOtpTokensTable.userId, user.id))
        .catch(() => {});
      ...
      logAction({
        userId: user.id,
        action: "register",
        details: { email, smsFallbackToEmail: true },
        ip: req.ip,
        userAgent: req.get("user-agent"),
      });
  ...
  await db
    ...
    .values({ userId: user.id, code: hashOtp(code), expiresAt });
...
router.post("/auth/phone-verify/confirm", async (req, res): Promise<void> => {
  ...
  if ((record.attempts ?? 0) >= MAX_OTP_ATTEMPTS) {
    res
      .status(429)
      .json({ error: "تجاوزت الحد الأقصى للمحاولات، يرجى طلب رمز جديد" });
    return;
  }
  if (record.code !== code) {
    await db
  ...
  try {
    ...
    logger.error(
      ...
      "Unable to save phone verification session",
    );
...
router.post("/auth/phone-verify/resend", async (req, res): Promise<void> => {
  ...
  if (secondsSinceLastSend < 60) {
    res.status(429).json({
      error: "انتظري دقيقة واحدة قبل طلب رمز جديد",
      secondsRemaining: Math.ceil(60 - secondsSinceLastSend),
    });
    return;
  }
  ...
  await db.insert(phoneOtpTokensTable).values({
    userId: user.id,
  ...
  try {
    ...
    await sendSms({
      to: normalizedPhone,
      body: `رمز التحقق في رباب: ${newCode} — صالح 10 دقائق`,
    });
  } catch (err) {
    if (process.env.NODE_ENV === "production") {
      res
        .status(503)
        .json({ error: "فشل إرسال رمز التحقق، يرجى المحاولة مرة أخرى" });
      return;
    }
...
router.post("/auth/verify-email", async (req, res): Promise<void> => {
  ...
  if (!checkRateLimit(verifyByIp, ip, VERIFY_MAX_PER_IP, VERIFY_WINDOW_MS)) {
    res
      .status(429)
      .json({ error: "تجاوزتِ الحد المسموح به من المحاولات، حاولي لاحقاً" });
    return;
  }
  ...
  if (
    failEntry &&
    failEntry.count >= OTP_MAX_FAILURES &&
    Date.now() - failEntry.since <= OTP_FAILURE_WINDOW_MS
  ) {
    res.status(429).json({
      error: "تم تجاوز عدد المحاولات المسموح بها، اطلبي رمزاً جديداً",
    });
    return;
  }
  ...
  if (!record) {
    ...
    if (failures >= OTP_MAX_FAILURES) {
      ...
      logAction({
        userId: user.id,
        action: "otp_locked",
        details: { email: normalizedEmail, failures },
        ip,
        userAgent: req.get("user-agent"),
      });
      res.status(429).json({
  ...
  try {
    ...
    logger.error(
      { err: error, userId: user.id },
      "Unable to save email verification session",
    );
    res.status(500).json({ error: "خطأ في حفظ الجلسة" });
    return;
  }
...
router.post("/auth/resend-verification", async (req, res): Promise<void> => {
  ...
  if (
    ...
    res
      .status(429)
      .json({ error: "تم إرسال الرمز مؤخراً، انتظري قليلاً قبل الطلب مجدداً" });
    return;
  }
...
router.post("/auth/login", async (req, res): Promise<void> => {
  ...
  if (!user.phoneVerified && user.phone) {
    ...
    try {
      ...
      logger.warn(
        { err: (err as any)?.message },
        "[SMS] Failed to send login OTP — proceeding without verification",
      );
    }
...
router.post("/auth/logout", async (req, res): Promise<void> => {
  ...
  if (authHeader?.startsWith("Bearer ")) {
    ...
    try {
      ...
      /* invalid/expired token — skip blocklist */
    }
  }
  req.session!.destroy(() => {
